...
class MIQPPlanner:
    ...
    def mode_is_active(self, mode_id: int) -> np.ndarray:
        ...
        if mode_id in range(2): # todo: originally 3
    ...
    def get_min_duration(self):
        ay_max = np.sqrt(self.params.a_max_sq - self.params.a_max ** 2)
        distance = self.params.lane_width
        t_min = np.sqrt(2 * distance / ay_max) + self.params.delta_steer
        return t_min
...
if __name__ == '__main__':
    ...
    init_state_curvlinear = CurvState(s=1.0, ds=5.0, dds=0.0)  # example
    ...
    s_constraints = [np.array([[0, 20]]*30),  # initial lane
