...
from testcaseopt.common.mat import compute_orientation_from_polyline
...
class GurobiStatelist:
    def __init__(self, model:Model, var_u, var_x, n_time_steps: int, eg_motion, curv_sys:CurvilinearCoordinateSystem, dt:float):
        ...
        self.eg_motion = eg_motion
        self.curv_sys = curv_sys
        self.model = model
        self.dt = dt
        self.var_u = var_u
        self.var_x = var_x
        self.state_list: List[CurvState] = self._init_state_list()
    ...
    def compute_lateral_offset_of_lane(self, target_center_line: np.ndarray, time_interval: Interval) -> np.ndarray:
        ...
        for vert in target_center_line:
            try:
                pos_map.append(self.curv_sys.convert_to_curvilinear_coords(vert[0], vert[1]))
            except:
                if len(pos_map)>0:
...
class MIQPPlanner:
    def __init__(self, s0: CurvState, n_time_steps: int, min_duration:List[float], dt:float, curv_sys: CurvilinearCoordinateSystem, params:ScenarioParams=ScenarioParams):
        ...
        self.dt = dt
        self.params = params
        self._init_mode_variables()
        self.var_x, self.var_u = self._init_state_variables(self.params.s_bounds, self.model, self.s0)
    ...
    def _init_state_variables(self, bounds_s, model: Model, s0:CurvState):
        ...
        # states
        var_x = np.empty([self.params.nx,self.n_time_steps+1],dtype=object)
        for t in range(self.n_time_steps + 1):
            for i_x in range(self.params.nx):
                if i_x == 0:
                    lb = bounds_s[0] + self.params.s_min[1] * self.params.dt * t
                    ub = bounds_s[1] + max(self.params.s_max[1], self.params.v_max) * self.params.dt * t
                else:
                    lb = self.params.s_min[i_x]
    ...
    def _init_costs(self, states: GurobiStatelist, slack_variables=None):
        ...
        # TODO: add costs for slack variables for s_constraints
        costs = QuadExpr()
        w_j = 0.01
        w_a = 10
        for state in states.state_list:
            costs.add(state.dds * (state.dds) * w_a)  # squared
    ...
    def set_long_constraints(self, s_constraints: List[np.ndarray]):
        ...
        for mode_id, s_minmax in enumerate(s_constraints):
            b_active = self.mode_is_active(mode_id)
            for t, b_tmp in enumerate(b_active):
                # TODO: add slack variables for s_constraints
...
dt=0.1
nT=30
init_state_curvlinear = CurvState(s=0.0, ds=5.0, dds=0.0) # example
curv_sys = CurvilinearCoordinateSystem# TODO: insert CurvilinearCoordinateSystem
target_lanelet = # TODO: insert target lane as Lanelet

planner = MIQPPlanner(init_state_curvlinear, n_time_steps=nT, min_duration=[0.2, 1.0, 0.2], dt=dt, curv_sys=curv_sys)
# longitudinal constraints
s_constraints = [np.ndarray([[0,20],[0,20],[0,20],[0,20],[0,20],[0,20],[0,20]]), # initial lane
                 np.ndarray([[5,20],[5,20],[5,20],[5,20],[5,20],[5,20],[5,20]]), # during lane change
                 np.ndarray([[5,30],[5,30],[5,30],[5,30],[5,30],[5,30],[5,30]])] # target lane
